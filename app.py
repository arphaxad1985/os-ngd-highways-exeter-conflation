...
if mode == "Demo (Exeter sample)":
    ...
    roadlink_file = st.sidebar.file_uploader(
        ...
        help="The RoadLink feature type from OS MasterMap Highways Network — Roads.",
    )
...
with tab1:
    ...
    with col_a:
        if os.path.exists("figures/qgis_matchstatus_map.png"):
            st.image("figures/qgis_matchstatus_map.png",
                     caption="QGIS — RoadLinks styled by matchStatus")
    with col_b:
        if os.path.exists("figures/python_matchstatus_map.png"):
            st.image("figures/python_matchstatus_map.png",
                     caption="Python (Matplotlib) — same data, reproducible")
...
with tab2:
    ...
    with col2:
        if os.path.exists("figures/conflation_status.png"):
            st.image("figures/conflation_status.png",
                     caption="Conflation status distribution")
        else:
...
with tab3:
    ...
    with col_m1:
        if os.path.exists("figures/qgis_unmatched_only.png"):
            st.image("figures/qgis_unmatched_only.png",
                     caption="QGIS — unmatched links only")
    with col_m2:
        if os.path.exists("figures/python_unmatched_only.png"):
            st.image("figures/python_unmatched_only.png",
                     caption="Python (Matplotlib) — unmatched links only")
    ...
    with col1:
        ...
        st.dataframe(unmatched["roadClassification"].value_counts(dropna=False).to_frame(),
                     use_container_width=True)
        st.markdown("**By provenance**")
        st.dataframe(unmatched["provenance"].value_counts(dropna=False).to_frame(),
                     use_container_width=True)
    with col2:
        st.markdown("**By form of way**")
        st.dataframe(unmatched["formOfWay"].value_counts(dropna=False).to_frame(),
                     use_container_width=True)

    if len(unmatched) > 0:
        st.markdown("### Length distribution")
        if os.path.exists("figures/unmatched_length_histogram.png"):
            st.image("figures/unmatched_length_histogram.png",
                     caption="Length distribution of unmatched links")
        else:
        ...
        st.dataframe(top10[["roadName", "roadClassification", "formOfWay", "length"]],
                     use_container_width=True)
...
with tab4:
    ...
    st.dataframe(disc[["roadName", "roadClassification", "formOfWay", "length"]],
                 use_container_width=True)
    if len(disc) > 0:
        st.markdown("**Clustering by road name**")
        st.dataframe(disc["roadName"].value_counts(dropna=False).to_frame(),
                     use_container_width=True)
...
with tab5:
    ...
    st.dataframe(review[["roadName", "roadClassification", "formOfWay", "length"]],
                 use_container_width=True)
...
with tab6:
    ...
    preview_cols = ["identifier", "roadName", "roadClassification",
                    "formOfWay", "matchStatus", "length"]
